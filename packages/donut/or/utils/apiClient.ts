import type { ModelInfo, OpenRouterModel, OpenRouterResponse } from "../types"
...
export class ApiClient {
  ...
  async fetchModels(forceRefresh = false): Promise<ModelInfo[]> {
    ...
    try {
      ...
      const models = this.filterAvailableModels(data.data).map((model) =>
  ...
  async testModel(
    modelId: string,
    apiKey: string,
    testMessage: string = "Hello! Please respond with a brief greeting.",
  ): Promise<{
    success: boolean
    response?: string
    error?: string
  ...
  private filterAvailableModels(models: OpenRouterModel[]): OpenRouterModel[] {
    return models.filter((model) => {
      ...
      if (!model.context_length || model.context_length <= 0) {
        console.debug(
          `Filtered out model ${model.id}: invalid context length (${model.context_length})`,
        )
        return false
      }
      ...
      if (model.per_request_limits) {
        const promptTokens = parseInt(
          model.per_request_limits.prompt_tokens || "0",
          10,
        )
        const completionTokens = parseInt(
          model.per_request_limits.completion_tokens || "0",
          10,
        )
  ...
  private isModelDeprecated(model: OpenRouterModel): boolean {
    ...
    if (
      model.architecture?.instruct_type?.toLowerCase().includes("deprecated")
    ) {
      return true
    }
    ...
    if (model.description) {
      ...
      if (
        deprecationKeywords.some((keyword) => description.includes(keyword))
      ) {
        return true
      }
    }
  ...
  private calculateCostTier(pricing: {
    prompt: string
    completion: string
  ...
  private getFallbackModels(): ModelInfo[] {
    return [
      ...
      },
      {
        ...
        costTier: "high",
        description:
          "Advanced reasoning model with enhanced thinking capabilities",
