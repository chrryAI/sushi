...
import type { FilterState, ModelInfo, SortConfig } from "../types"
...
export function useFiltering(models: ModelInfo[]): UseFilteringReturn {
  ...
  const setSort = useCallback(
    (key: keyof ModelInfo | null, direction: "asc" | "desc" = "asc") => {
      setSortConfig((prev) => {
        if (prev.key === key) {
  ...
  const isExperimentalModel = useCallback((model: ModelInfo) => {
    ...
    return (
      experimentalPatterns.some((pattern) => pattern.test(model.id)) ||
