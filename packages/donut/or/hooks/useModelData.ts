...
export function useModelData(
  ...
  const fetchModels = useCallback(
    async (forceRefresh = false) => {
      try {
        ...
        const { freeModels: free, paidModels: paid } =
          apiClient.categorizeModels(fetchedModels)
        setFreeModels(free)
        setPaidModels(paid)
      } catch (err) {
        const errorMessage =
        ...
        if (fallbackModels && fallbackModels.length > 0) {
          ...
          const { freeModels: free, paidModels: paid } =
            apiClient.categorizeModels(fallbackModels)
          setFreeModels(free)
          setPaidModels(paid)
        }
      } finally {
  ...
  const testModel = useCallback(
    async (modelId: string, apiKey: string, testMessage?: string) => {
