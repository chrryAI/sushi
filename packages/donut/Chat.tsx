...
import { buildAiRequestBody, handleAiResponse } from "./lib"
...
import { WatermelonButton } from "./Skeleton"
import {
  type aiAgent,
  type collaboration,
  emojiMap,
...
export default function Chat(props: {
  ...
  const { slug, appStatus, minimize, setMinimize } = useApp()
  ...
  const canShowMachine =
    (!showTribe && empty && minimize && !threadId) || isAgentModalOpen
  ...
  const playAIResponseWithTTS = async (text?: string) => {
    // return
    try {
      if (canEarnBadgeRef.current)
        await auth.patchCharacterProfile({
          newProfile: true,

          onSuccess: () => {
            setCanEarnBadge(false)
          },
          onError: () => {
            setCanEarnBadge(false)
          },
  ...
  const handleSubmit = async (approve?: boolean) => {
    ...
    if (creditsLeft === 0 && selectedAgent?.name !== "free") {
    ...
    try {
      ...
      const { body: requestBody, headers: requestHeaders } = buildAiRequestBody(
        {
          token,
          files,
          slug,
      ...
      // Handle error and JSON responses
      const aiResult = await handleAiResponse(agentResponse)
      if (!aiResult.ok) return
      if (aiResult.result?.success) return
    } catch (error: any) {
  ...
  const [isPrivacyApproved, setIsPrivacyApprovedInternal] = useLocalStorage(
    "isPrivacyApproved",
    !!user,
  )

  const canEarnBadgeRef = useRef(false)
  const setCanEarnBadge = (value: boolean) => {
    !auth.characterProfilesEnabled && (canEarnBadgeRef.current = value)
  }
  const setIsPrivacyApproved = (value: boolean) => {
    if (value && !auth.characterProfilesEnabled) {
      setCanEarnBadge(true)
    }
    setIsPrivacyApprovedInternal(value)
  }
  ...
  const isVoiceDisabled = isLoading || disabled
  ...
  const showAgentDetailsInitial = creditsLeft ? selectedAgent?.name : "free"
  const [showAgentDetails, setShowAgentDetails] = useState<string | undefined>(
    showAgentDetailsInitial,
  )

  useEffect(() => {
    setShowAgentDetails(showAgentDetailsInitial)
  }, [selectedAgent])
  ...
  const architect = () => {
    if (!canShowMachine) return
    return (
      <Div
        style={{
          ...styles.modalContent.style,
          position: "relative",
          gap: 2.5,
        }}
      >
        ...
        >
          ...
          <Div
            style={{
              ...
              marginTop: ".5rem",
              flexWrap: "wrap",
              // alignItems: !isAgentModalOpen?"center":undefined,
              alignItems: "center",
              // flexDirection:!isAgentModalOpen?'row':'column',
              justifyContent: "center",
            }}
          >
            {showAgentDetails &&
                ...
                <Div key={agent.name}>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      isAgentModalOpen && setIsAgentModalOpen(false)
                    }}
                  >
                    {agent.name === "deepSeek" ? (
                      <DeepSeek color={COLORS.blue} size={18} />
                    ) : agent.name === "chatGPT" ? (
                      <OpenAI size={18} />
                    ) : agent.name === "claude" ? (
                      <Claude color={COLORS.orange} size={18} />
                    ) : agent.name === "gemini" ? (
                      <Gemini color={COLORS.purple} size={18} />
                    ) : agent.name === "flux" ? (
                      <Flux size={18} />
                    ) : agent.name === "perplexity" ? (
                      <Perplexity color={COLORS.blue} size={18} />
                    ) : agent.name === "sushi" ? (
                      <Img slug="sushi" size={18} />
                    ) : agent.name === "grok" ? (
                      <Grok size={18} />
                    ) : agent.name === "peach" ? (
                      <Img slug="peach" size={18} />
                    ) : agent.name === "free" ? (
                      <OpenRouter size={18} />
                    ) : null}
                    {!isAgentModalOpen ? "" : agent.displayName || agent.name}
                  </Button>
                </Div>
              ))}
          </Div>
          {empty && !threadIdRef.current && !burn ? (
            <Div
              style={{
                ...
                top: ".2rem",
                marginLeft: "auto",
                // top: !isChatFloating ? (showQuotaInfo ? 0 : 32) : -10,
                zIndex: 50,
              }}
            >
              {isRetro && (
  ...
  const getFilteredAgents = (onlyOne?: boolean) =>
    aiAgents
      ?.filter(
        (agent) =>
          (agent?.name === "sushi" || agent?.name === "peach" ? true : true) &&
          (isDebateAgentModalOpen
            ? onlyOne
              ? agent.name === showAgentDetails
              : files.length
      ...
      >
        ...
        {!thread && showSuggestions && !isGame && (
          <Div
            style={{
              marginBottom:
                minimize && !isMobileDevice ? 100 : minimize ? 0 : 10,
            }}
          >
            <Div
              style={{
            ...
            <App isJester={!canShowMachine} />
          </Div>
        )}
        <Div
          style={{
            opacity: isAgentModalOpen ? 0 : 1,
            marginBottom: minimize ? 7.5 : 0,
            display: "flex",
            gap: "1rem",
            alignItems: "center",
          }}
        ...
        >
          ...
          >
              ...
              <Div style={styles.filePreviewArea.style}>
                {files.map((file, index) => {
                  ...
                  return (
                    <Div
                      key={file.name}
                      style={{ ...styles.filePreview.style, gap: 5 }}
                    >
                      {isImage ? (
                        <Img
            ...
            {needsReview && (
              ...
              >
                <P style={{ color: "var(--shade-6)", fontSize: ".95rem" }}>
                  {t(placeholder || "")}
                </P>
                {!auth.characterProfilesEnabled && (
                  <Div>
                    <Button
                      onClick={() => setEarnBadge(!earnBadge)}
                      className="transparent"
                      style={{ fontSize: ".95rem" }}
                    >
                      <Img slug="coder" size={16} />
                      {t("Earn a Badge")}
                      {earnBadge && (
                        <CircleCheck color={COLORS.green} size={16} />
                      )}
                    </Button>
                  </Div>
                )}
                  ...
                  >
                    <Button
                      ...
                      style={{
                        ...
                        ...(onlyAgent ? styles.debateAgentButtonDisabled : {}),
                      }}
                    >
                      ...
                      ) : selectedAgent.name === "peach" ? (
                        <Img icon="peach" size={24} />
                      ) : selectedAgent.name === "free" ? (
                        <OpenRouter size={24} />
                      ) : null}
                      {onlyAgent ||
                      selectedAgent?.name === "flux" ||
                    ...
                    {debateAgent && !onlyAgent ? (
                      ...
                      >
                        ...
                        {debateAgent.name === "deepSeek" ? (
                          ...
                          <Img icon="sushi" size={22} />
                        ) : debateAgent.name === "peach" ? (
                          <Img icon="peach" size={24} />
                        ) : debateAgent.name === "free" ? (
                          <OpenRouter size={24} />
                        ) : null}
                      </Button>
                    ) : (
                ...
                {isAttaching ? (
                  <Span style={styles.attachButtons.style}>
                    ...
                    <Button
                      ...
                      title={t("Video")}
                      onClick={() => {
                        if (!selectedAgent?.capabilities.video) {
                          setIsAgentModalOpen(true)
                          return
                        }
                    ...
                    <Button
                      ...
                      title={t("Audio")}
                      onClick={() => {
                        if (!selectedAgent?.capabilities.audio) {
                          setIsAgentModalOpen(true)
                          return
                        }
          ...
          {(!isChatFloating || isIDE) && (
            ...
            >
              ...
              >
                {!hitHourlyLimit && <Coins color="var(--accent-1)" size={16} />}
                {hitHourlyLimit && !threadId ? (
                  ...
                  <Button
                    ...
                    title={t("credits", {
                      count:
                        Number(selectedAgent?.creditCost || "1") +
                        Number(debateAgent?.creditCost || "0"),
                    })}
                  >
                    {t("credits", {
                      count:
