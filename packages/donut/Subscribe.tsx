...
export default function Subscribe({
  ...
  // BTCPay invoice oluştur
  const createBtcpayInvoice = async () => {
    if (!selectedPlan || selectedPlan === "member") return
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || "Failed to create Bitcoin invoice")
      }
    } catch (err) {
      captureException(err)
      toast.error("Failed to create Bitcoin invoice")
    } finally {
      setIsBtcpayLoading(false)
    }
  }

  // BTCPay polling - ödeme takibi
          ...
          <>
            ...
            {canBuyCredits() || canSubscribe() ? (
              <>
                ...
                <Button
                  disabled={isBtcpayLoading || disabled}
                  className="small"
                ...
                {btcpayInvoice && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Invoice: {btcpayInvoice.invoiceId.slice(0, 16)}...
                      </p>
  ...
  const shouldShowGift = () => {
    const enabled = false
    if (isTribe || isMolt || !enabled) return false
