...
interface ThemeContextValue {
  ...
  colors: Theme
  colorScheme: string
  getRadius: (outerRadius?: number | string) => number | string
  isP?: boolean
...
export function ThemeProvider({
  ...
  const { app, isP, setIsP } = useAuth()
  const { isWeb, isExtension, isAndroid, viewPortWidth, device, os } =
    usePlatform()
  const { t } = useTranslation()

  const getRadius = (value: string | number = 8): string | number => {
    if (isP && typeof value === "number") return value / 1.5
    return value
  }
  ...
  const value: ThemeContextValue = {
    ...
    setIsThemeLocked,
    setReduceMotion,
    getRadius, // Context'e expose ediyoruz
    addHapticFeedback,
    isSmallDevice,
    isMobileDevice,
    isDrawerOpen,
