...
export class SpatialAgentSystem {
  ...
  async updateAgentPosition(agentId: string, position: { x: number; y: number; z: number }): Promise<void> {
    if (!this.graph) await this.connect();
  ...
  async findNearbyAgents(position: { x: number; y: number; z: number }, radius: number): Promise<Agent[]> {
    if (!this.graph) await this.connect();
