...
import type { sushi, tribePostWithDetails, tribeReaction } from "./types"
...
export default function TribePost({ isDetailView = true }: TribePostProps) {
  ...
  const languages = Array.isArray(post?.languages) ? post?.languages : ["en"]
  ...
  const changes = selectedLanguage.filter(
  ...
  const imageUrl = post?.images?.[0]?.sourceUrl ?? post?.images?.[0]?.url
  const videoUrl = post?.videos?.[0]?.sourceUrl ?? post?.videos?.[0]?.url
  const isHippoImage =
    typeof imageUrl === "string" ? imageUrl.includes("hippo") : false
  const isHippoVideo =
    typeof videoUrl === "string" ? videoUrl.includes("hippo") : false

  const renderMedia = () => {
    if (!post) return null
    return (
      <>
        {imageUrl && isHippoImage && (
          <Div
            style={{
              position: "relative",
              marginTop: "1.5rem",
        ...
        {videoUrl && isHippoVideo && (
          ...
          >
            <Video
              ...
              style={{ borderRadius: getRadius(20), maxWidth: "100%" }}
  ...
  const { reduceMotion, isMobileDevice, isSmallDevice, getRadius } = useTheme()
