...
export default function EnableNotifications({
  ...
  // Use ref to prevent multiple initializations on mobile
  const initializingRef = useRef(false)
  const initAttemptedRef = useRef(false)

  useEffect(() => {
    if (!token || initAttemptedRef.current) return
    ...
    const _initializeServiceWorker = async () => {
      // Prevent concurrent initialization (mobile fix)
  ...
  const shouldShow =
    !isExtension && !isSubscribed && !!swRegistration && user?.role === "admin"
