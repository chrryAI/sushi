...
export const users = pgTable(
  "user",
  {
    ...
    apiKeys: jsonb("apiKeys").$type<{
      ...
      xai?: string // Encrypted XAI API key
      s3?: string // Encrypted S3 API key
      sentry?: string // Encrypted S3 API key
    }>(),
    apiKey: text("apiKey"),
    image: text("image"),
    ...
    // Golden Ratio φ-Engine counters & config
    totalThreadCount: integer("totalThreadCount").default(0).notNull(),
    goldenRatioConfig: jsonb("goldenRatioConfig")
      .$type<{
        memory?: {
...
export const subscriptions = pgTable(
  "subscription",
  {
    ...
    provider: text("provider", {
      enum: ["stripe", "apple", "google", "btcpay", "crypto"],
    }).notNull(),
...
export const threads = pgTable("threads", {
  ...
  metadata: jsonb("metadata")
    .$type<{
      swarm?: swarm
      /** Last chopStick payload used in this thread — for traceability & replay */
      lastChopstickPayload?: {
        agentName?: string
        modelId?: string
...
export const branchAgents = pgTable("branchAgents", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("userId").references(() => users.id, { onDelete: "cascade" }),
  guestId: uuid("guestId").references(() => guests.id, { onDelete: "cascade" }),
  ...
  instructions: jsonb("instructions").$type<any[]>().default([]),
  memories: jsonb("memories").$type<any[]>().default([]),
  characterProfile: jsonb("characterProfile"),
  evolutionScore: real("evolutionScore").default(0),
  ...
  "messages",
  {
    ...
    metadata: jsonb("metadata")
      .$type<{
        ...
        swarm?: swarm
        systemPrompt?: string
        /** chopStick payload that generated this AI message — for billing & replay */
        chopstickPayload?: {
          agentName?: string
          modelId?: string
          keySource?: "byok" | "app_key" | "system_key" | "free"
...
export const streamLogs = pgTable(
  "streamLogs",
  {
    id: uuid("id").defaultRandom().notNull().primaryKey(),
    userId: uuid("userId").references(() => users.id, { onDelete: "set null" }),
    ...
    /** The actual OpenRouter model string e.g. "deepseek/deepseek-v3.2" */
    modelId: text("modelId").notNull(),
    /** Where the OR key came from */
    keySource: text("keySource", {
      enum: ["byok", "app_key", "system_key", "free"],
    }).notNull(),
    tokensIn: integer("tokensIn").notNull().default(0),
    tokensOut: integer("tokensOut").notNull().default(0),
    /** Estimated USD cost based on internal pricing table */
    costUsd: numeric("costUsd", { precision: 12, scale: 8 })
      .notNull()
      .default("0"),
    isDegraded: boolean("isDegraded").notNull().default(false),
    isBYOK: boolean("isBYOK").notNull().default(false),
    createdOn: timestamp("createdOn", { mode: "date", withTimezone: true })
      .defaultNow()
      .notNull(),
  },
  (table) => [
    index("stream_logs_user_date_idx").on(table.userId, table.createdOn),
    index("stream_logs_guest_date_idx").on(table.guestId, table.createdOn),
    index("stream_logs_model_date_idx").on(table.modelId, table.createdOn),
    index("stream_logs_app_date_idx").on(table.appId, table.createdOn),
  ],
)
...
export const newsArticles = pgTable(
  "newsArticles",
  {
    ...
    // Search
    embedding: vector("embedding", { dimensions: 4096 }), // For semantic search
...
export const tasks = pgTable("task", {
  ...
  modifiedOn: timestamp("modifiedOn", { mode: "date", withTimezone: true })
    .defaultNow()
    .notNull(),
  kanbanBoardId: uuid("kanbanBoardId").references(() => userKanbanBoards.id, {
    onDelete: "cascade",
  }),
  ...
  // Labels for categorization (e.g., "bug", "feature", "urgent")
  labels: jsonb("labels").$type<string[]>().default([]),
  // Label colors map for UI rendering
  labelColors: jsonb("labelColors").$type<Record<string, string>>().default({}),
})
...
export const codeEmbeddings = pgTable(
  "codeEmbeddings",
  {
    ...
    embedding: vector("embedding", { dimensions: 4096 }), // OpenAI text-embedding-3-small
...
// ============================================================================
// CODEBASE ISSUES - Cached Sentry issues for AI context
// ============================================================================

export const codebaseIssues = pgTable(
  "codebaseIssues",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    issueId: text("issueId").notNull().unique(), // Sentry issue ID
    repoName: text("repoName").notNull(), // e.g., "chrryAI/vex"
    title: text("title").notNull(),
    count: integer("count"), // Error count
    userCount: integer("userCount"), // Affected user count
    firstSeen: timestamp("firstSeen", { mode: "date", withTimezone: true }),
    lastSeen: timestamp("lastSeen", { mode: "date", withTimezone: true }),
    level: text("level"), // error, warning, info
    status: text("status"), // unresolved, resolved, ignored
    culprits: text("culprits").array(), // Stack trace filenames
    metadata: jsonb("metadata"), // Full Sentry response for context
    fetchedAt: timestamp("fetchedAt", { mode: "date", withTimezone: true })
      .defaultNow()
      .notNull(), // Cache timestamp
    createdAt: timestamp("createdAt", { mode: "date", withTimezone: true })
      .defaultNow()
      .notNull(),
    updatedAt: timestamp("updatedAt", { mode: "date", withTimezone: true })
  ...
  (table) => ({
    issueIdIdx: index("codebaseIssues_issueId_idx").on(table.issueId),
    repoNameIdx: index("codebaseIssues_repoName_idx").on(table.repoName),
    fetchedAtIdx: index("codebaseIssues_fetchedAt_idx").on(table.fetchedAt),
    lastSeenIdx: index("codebaseIssues_lastSeen_idx").on(table.lastSeen),
    statusIdx: index("codebaseIssues_status_idx").on(table.status),
  }),
)

export type CodebaseIssue = typeof codebaseIssues.$inferSelect
export type NewCodebaseIssue = typeof codebaseIssues.$inferInsert
...
// ═════════════════════════════════════════════════════════════════════════════
// ₿ BTCPAY INVOICES - Bitcoin Payment Processing
// ═════════════════════════════════════════════════════════════════════════════

export const btcpayInvoices = pgTable(
  "btcpay_invoices",
  {
    id: text("id").primaryKey(), // BTCPay invoice ID
    userId: uuid("userId").references(() => users.id, { onDelete: "set null" }),
    guestId: uuid("guestId").references(() => guests.id, {
      onDelete: "set null",
    }),
    ...
    status: text("status", {
      enum: ["New", "Processing", "Settled", "Expired", "Invalid"],
    })
      .notNull()
      .default("New"),
  ...
  (table) => [
    index("btcpay_invoices_user_idx").on(table.userId),
    index("btcpay_invoices_guest_idx").on(table.guestId),
    index("btcpay_invoices_status_idx").on(table.status),
    index("btcpay_invoices_created_idx").on(table.createdAt),
  ],
)

export type BtcpayInvoice = typeof btcpayInvoices.$inferSelect
export type NewBtcpayInvoice = typeof btcpayInvoices.$inferInsert
...
// ─── API Requests ─────────────────────────────────────────────────────────────
// Her Cloudflare API çağrısını loglar (başarılı veya başarısız)

export const cfApiRequests = pgTable(
  "cf_api_requests",
  {
    id: uuid("id").defaultRandom().primaryKey(),

    // Hangi hesap / zone ile ilgili
    accountId: text("account_id"),
    zoneId: text("zone_id"),

    // İstek bilgileri
    method: httpMethodEnum("method").notNull(),
    endpoint: text("endpoint").notNull(), // örn. "/zones", "/accounts/:id/workers/scripts"
    sdkMethod: text("sdk_method"), // örn. "client.zones.create"

    // Yanıt bilgileri
    statusCode: integer("status_code"),
    success: boolean("success").notNull().default(false),
    durationMs: integer("duration_ms"), // istek süresi (ms)
    ...
    // İstek / yanıt boyutu (opsiyonel, debug için)
    requestBodySize: integer("request_body_size"), // bytes
    responseBodySize: integer("response_body_size"), // bytes
  ...
  (t) => [
    index("cf_api_requests_account_idx").on(t.accountId),
    index("cf_api_requests_zone_idx").on(t.zoneId),
    index("cf_api_requests_endpoint_idx").on(t.endpoint),
    index("cf_api_requests_created_at_idx").on(t.createdAt),
    index("cf_api_requests_success_idx").on(t.success),
  ],
)

// ─── Rate Limit Events ────────────────────────────────────────────────────────
// 429 alındığında ayrıca kaydedilir, throttle analizi için

export const cfRateLimitEvents = pgTable(
  "cf_rate_limit_events",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    requestId: uuid("request_id").references(() => cfApiRequests.id, {
      onDelete: "set null",
    }),
    ...
    // Cloudflare'in döndürdüğü Retry-After (saniye)
  ...
  (t) => [
    index("cf_rate_limit_events_endpoint_idx").on(t.endpoint),
    index("cf_rate_limit_events_created_at_idx").on(t.createdAt),
  ],
)

// ─── Zones ────────────────────────────────────────────────────────────────────
// Üzerinde işlem yapılan Cloudflare zone'larının önbelleği

export const cfZones = pgTable(
  "cf_zones",
  {
    id: text("id").primaryKey(), // Cloudflare zone ID
    name: text("name").notNull(), // örn. "example.com"
    accountId: text("account_id").notNull(),
    type: text("type").notNull(), // "full" | "partial" | "secondary"
  ...
  (t) => [
    index("cf_zones_account_idx").on(t.accountId),
    index("cf_zones_name_idx").on(t.name),
  ],
)

// ─── SDK Sessions ─────────────────────────────────────────────────────────────
// Uygulama ayağa kalktığında bir session açılır; toplam kullanım izlenebilir

export const cfSdkSessions = pgTable("cf_sdk_sessions", {
  id: uuid("id").defaultRandom().primaryKey(),

  // Hangi token / credential kullanıldı (hash'ini sakla, token'ı değil!)
  apiTokenHash: text("api_token_hash"), // SHA-256 of token
  sdkVersion: text("sdk_version"), // örn. "5.2.0"
  nodeVersion: text("node_version"),
  runtime: text("runtime"), // "node" | "deno" | "bun" | "worker" | ...

  totalRequests: bigint("total_requests", { mode: "number" })
    .notNull()
    .default(0),
  totalErrors: bigint("total_errors", { mode: "number" }).notNull().default(0),
  totalRetries: bigint("total_retries", { mode: "number" })
    .notNull()
    .default(0),
...
export const userKanbanBoards = pgTable(
  "user_kanban_boards",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    userId: uuid("user_id").notNull(),
    title: text("title").notNull().default("My App Dojo"),
  ...
  (t) => [index("kanban_boards_user_idx").on(t.userId)],
)
...
export const kanbanColumns = pgTable(
  "kanban_columns",
  {
    ...
    boardId: uuid("board_id")
      .notNull()
      .references(() => userKanbanBoards.id, { onDelete: "cascade" }),
    title: text("title").notNull(), // e.g. Discovery, Active, Mastery, Archived
    order: integer("order").notNull().default(0),
    config: jsonb("config").$type<{
      color?: string
      maxCards?: number
...
export const kanbanCards = pgTable(
  ...
  (t) => [
    ...
    index("kanban_cards_order_idx").on(t.columnId, t.order),
  ],
)

// ─── Relations ────────────────────────────────────────────────────────────────

export const userKanbanBoardsRelations = relations(
  userKanbanBoards,
  ({ many }) => ({
...
export const kanbanColumnsRelations = relations(
  kanbanColumns,
  ({ one, many }) => ({
    board: one(userKanbanBoards, {
      fields: [kanbanColumns.boardId],
      references: [userKanbanBoards.id],
    }),
...
export const cfApiRequestsRelations = relations(cfApiRequests, ({ many }) => ({
  rateLimitEvents: many(cfRateLimitEvents),
}))

export const cfRateLimitEventsRelations = relations(
  cfRateLimitEvents,
  ({ one }) => ({
    request: one(cfApiRequests, {
      fields: [cfRateLimitEvents.requestId],
      references: [cfApiRequests.id],
    }),
  }),
)
...
export type CFApiRequest = typeof cfApiRequests.$inferSelect
export type NewCFApiRequest = typeof cfApiRequests.$inferInsert

export type CFRateLimitEvent = typeof cfRateLimitEvents.$inferSelect
export type NewCFRateLimitEvent = typeof cfRateLimitEvents.$inferInsert

export type CFZone = typeof cfZones.$inferSelect
export type NewCFZone = typeof cfZones.$inferInsert

export type CFSDKSession = typeof cfSdkSessions.$inferSelect
export type NewCFSDKSession = typeof cfSdkSessions.$inferInsert

export type UserKanbanBoard = typeof userKanbanBoards.$inferSelect
export type NewUserKanbanBoard = typeof userKanbanBoards.$inferInsert
