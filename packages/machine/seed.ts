...
async function createHumans() {
  ...
  if (!admin) {
    ...
    admin = await createUser(adminPayload)
  ...
  if (!feedback) {
    ...
    feedback = await createUser(feedbackPayload)
  } else {
  ...
  if (!diplomatic) {
    ...
    diplomatic = await createUser(diplomaticPayload)
  ...
  if (!localswaphub) {
    ...
    localswaphub = await createUser(localswaphubPayload)
  ...
  return { feedback, admin, localswaphub }
}
...
const create = async () => {
  ...
  const { feedback, admin, localswaphub } = await createHumans()
  ...
  for (const event of analyticsEvents) {
    const eventTime = new Date(now.getTime() - event.hoursAgo * 60 * 60 * 1000)
  ...
  for (const timer of timerData) {
    const timerDate = new Date(now.getTime() - timer.hoursAgo * 60 * 60 * 1000)
  ...
  for (const msg of feedbackMessages) {
    const msgDate = new Date(now.getTime() - msg.daysAgo * 24 * 60 * 60 * 1000)
...
const prod = async () => {
  ...
  const { admin } = await createHumans()
  if (!admin) throw new Error("Admin user not found")
