...
pnpm dev            # Runs api (port 3001) + chrry (port 3000)
```
...
- Barrel files are common in packages (e.g., `packages/donut/index.ts`).
- Path aliases: `chrry/*` maps to `packages/donut/*` at the root tsconfig level.
...
- **Vitest** — Primary unit/integration runner (apps/api, apps/chrry, packages/donut, packages/vault, packages/machine).
...
- **UI tests** use `happy-dom` (not jsdom) with `globals: true` and a comprehensive mock context fixture at `packages/donut/__tests__/mocks/mockContexts.tsx`.
...
- User-provided keys are encrypted at rest (`packages/machine/encryption.ts`).
...
`packages/machine/` is the single source of truth for all data access.

- **ORM:** Drizzle ORM with `postgres-js` driver
- **Schema:** Monolithic `src/schema.ts` (~6,300 lines, 112 tables)
- **Migrations:** 265+ Drizzle-generated SQL files in `packages/machine/drizzle/`
