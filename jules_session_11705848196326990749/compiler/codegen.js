...
const lookup = (scope, name, failEarly = false) => {
  ...
  if (local?.metadata?.captured) {
    const capturedArr = Array.from(scope._captured ?? []);
    const captureIdx = capturedArr.indexOf(name);

    if (captureIdx !== -1) {
      // It is in THIS scope's context
      const offset = 8 + captureIdx * 9;
      return [
        [ Opcodes.local_get, scope.locals['#context'].idx ],
        Opcodes.i32_to_u,
        [ Opcodes.load, 0, ...unsignedLEB128(offset) ]
      ];
    }
  }

  if (local?.idx === undefined) {
    // Check for capture in parent scopes
    let depth = 0;
    let targetScope = scope;
    let parentCount = 0;

    while (targetScope && targetScope.name !== '#main') {
      // Check if this scope captures it
      const capturedArr = Array.from(targetScope._captured ?? []);
      const captureIdx = capturedArr.indexOf(name);

      if (targetScope._captured) console.log(`checking ${name} in ${targetScope.name} (parent: ${targetScope.parent?.name}, captured: ${[...targetScope._captured]})`);

      if (captureIdx !== -1) {
        // Found it in this scope's context!
        // Generate code to traverse up `parentCount` times and read from context
        const out = [
          [ Opcodes.local_get, scope.locals['#context'].idx ],
          Opcodes.i32_to_u
        ];
        ...
        for (let i = 0; i < parentCount; i++) {
          out.push(
            [ Opcodes.i32_load, 0, 4 ] // load parent ctx
          );
        }
...
const generateCall = (scope, decl, _global, _name, unusedValue = false) => {
  ...
  if (func && func._captured) {
    // pass current context as first arg to child function
    // if current function is main (no context), pass 0 (null)
    if (scope.name === '#main' || !scope.locals['#context']) {
      out.push(
        number(0, valtypeBinary),
        number(TYPES.object, Valtype.i32)
      );
    } else {
      out.push(
        [ Opcodes.local_get, scope.locals['#context'].idx ],
        [ Opcodes.local_get, scope.locals['#context#type'].idx ]
      );
    }
    paramOffset += 2;
  }
  ...
  if (func && func.constr) {
    out.push(
      ...forceDuoValtype(scope, decl._newTargetWasm ?? createNewTarget(scope, decl, idx - importedFuncs.length), func.params[paramOffset]),
      ...forceDuoValtype(scope, decl._thisWasm ?? createThisArg(scope, decl), func.params[paramOffset + 2])
    );
    paramOffset += 4;
  }

  if (func && func.method) {
    out.push(...forceDuoValtype(scope, decl._thisWasm ?? createThisArg(scope, decl), func.params[paramOffset]));
    paramOffset += 2;
  }
...
const generateAssign = (scope, decl, _global, _name, valueUnused = false) => {
  ...
  if (local === undefined) {
    ...
    while (targetScope && targetScope.name !== '#main') {
      ...
      if (captureIdx !== -1) {
        // found captured variable, assign to it
        const offset = 8 + captureIdx * 9;

        // get context pointer and traverse up
        const ctxPtr = localTmp(scope, '#assign_ctx_ptr', Valtype.i32);
        const out = [
          [ Opcodes.local_get, scope.locals['#context'].idx ],
          Opcodes.i32_to_u
        ];
        ...
        out.push(
          [ Opcodes.local_tee, ctxPtr ],
          ...(op === '=' ? generate(scope, decl.right) : performOp(scope, op, [
            [ Opcodes.local_get, ctxPtr ],
            [ Opcodes.load, 0, ...unsignedLEB128(offset) ]
          ], generate(scope, decl.right), [
            [ Opcodes.local_get, ctxPtr ],
            [ Opcodes.i32_load8_u, 0, ...unsignedLEB128(offset + ValtypeSize[valtype]) ]
          ], getNodeType(scope, decl.right))),
          // todo: tee value if not unused?
        ...
        if (!valueUnused) out.push(number(UNDEFINED)); // todo: return actual value
        return out;
      }
...
const generateFunc = (scope, decl, forceNoExpr = false) => {
  ...
  const func = {
    ...
    generate() {
      ...
      if (func._captured && func._captured.size > 0) {
        // create context
        // 4 bytes id, 4 bytes parent ctx, captured vars
        const contextSize = 8 + func._captured.size * 9;
        const ctxTmp = localTmp(func, '#context_tmp', Valtype.i32);

        wasm.unshift(
          number(contextSize, Valtype.i32),
          [ Opcodes.call, includeBuiltin(func, '__Porffor_malloc').index ],
          [ Opcodes.local_tee, ctxTmp ],

          // store context id (using func index as "type" for now for debug)
          number(func.index, Valtype.i32),
          [ Opcodes.i32_store, 0, 0 ],

          // store parent context (arg 0 if captured, else 0)
          [ Opcodes.local_get, ctxTmp ],
          ...(func.name === '#main' || !func.locals['#context'] ? [
            number(0, Valtype.i32)
          ] : [
            [ Opcodes.local_get, func.locals['#context'].idx ],
            Opcodes.i32_to_u
          ]),
          [ Opcodes.i32_store, 0, 4 ],

          // set current context local
          [ Opcodes.local_get, ctxTmp ],
          Opcodes.i32_from_u,
          [ Opcodes.local_set, func.locals['#context']?.idx ?? localTmp(func, '#context', valtypeBinary) ],

          // set context type local
          number(TYPES.object, Valtype.i32),
          [ Opcodes.local_set, func.locals['#context#type']?.idx ?? localTmp(func, '#context#type', Valtype.i32) ]
        );

        // populate context with arguments and locals if they are captured
        // we need to iterate args and check if they are captured
        // wait, params are in `args` array in `generateFunc`
        // we can iterate that
      }

      // Populate context with captured params
      // This needs to be done *after* we generate the function body prologue (where args are processed)
      // BUT `wasm` array is prepended with `preface`.
      // `generateFunc` logic is a bit split.
      // `args` processing happens inside `generateFunc` but before we generate body.
      // We can iterate `args` here (it's in closure of `generateFunc`)

      if (func._captured && func._captured.size > 0) {
        // iterate over params
        let paramOffset = 0;
        // params are in `func.params`? No, `args` has metadata.
        // `func.params` is wasm types.

        for (let i = 0; i < args.length; i++) {
          const arg = args[i];
          const name = arg.name;
          if (func._captured.has(name)) {
            // copy param to context
            const capturedArr = Array.from(func._captured ?? []);
            const captureIdx = capturedArr.indexOf(name);
            const offset = 8 + captureIdx * 9;

            // Param is already in a local (allocated in `generateFunc`)
            ...
            if (local) {
              wasm.push(
                ...
                [ Opcodes.local_get, func.locals['#context'].idx ],
                ...(local.type === Valtype.f64 ? [
                  [ Opcodes.local_get, func.locals[name + '#type'].idx ]
                ] : [
                  // if param is i32 (unlikely for user vars), assume number type?
                  // arguments are usually valtypeBinary (f64) + i32 type
                  number(TYPES.number, Valtype.i32)
                ]),
                [ Opcodes.i32_store8, 0, ...unsignedLEB128(offset + ValtypeSize[valtype]) ]
              );
  ...
  const args = [];
  if (func._captured) args.push({ name: '#context', type: TYPES.array }); // context pointer
  ...
  func.params = new Array((params.length + (func.constr ? 2 : (func.method ? 1 : 0)) + (func._captured ? 1 : 0)) * 2).fill(0).map((_, i) => i % 2 ? Valtype.i32 : valtypeBinary);
