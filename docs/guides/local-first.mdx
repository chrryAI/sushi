...
description: "Architectural philosophy for building a local-first app using SWR, Dexie, and REST APIs."
---

Below is the architectural philosophy for building a local-first application using SWR, Dexie (IndexedDB), and REST APIs.

## 1. The philosophy: REST as an interface

Instead of leaking database queries (like Drizzle/Prisma) into the frontend components, this architecture treats **REST APIs as "God Functions"**.
...
This architecture uses a hybrid caching strategy to combine the benefits of both approaches: **Instant reads** (local) + **Fresh data** (network).
...
- **Hybrid cache provider:** Implemented with SWR + Dexie.
