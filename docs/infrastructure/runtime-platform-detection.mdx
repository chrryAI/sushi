...
description: "Architecture and API reference for the runtime platform detection system that enables a single codebase to run across web and native platforms."
---

## Overview

A single codebase runs on all platforms. There is no need for `.native.tsx` or `.web.tsx` files. The app detects the platform at runtime and adapts automatically.
