...
description: "How Redis caching is integrated into getUser() and getGuest() functions to reduce database load and improve API response times."
---

## Overview

Added Redis caching layer to `getUser()` and `getGuest()` functions to reduce database load and improve API response times.
...
- Cached requests: **1-5ms** (was 50-100ms)
...
- Cost savings: notable (if using managed DB)
...
### Phase 1: deploy and monitor (week 1)
...
- Significantly faster API responses (cached)
