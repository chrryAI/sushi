...
export async function remember(block: Omit<AgentMemoryBlock, "embedding">): Promise<void> {
...
export async function recall(
  ...
  const results = await db.similaritySearch(q, k, {
    filter: (doc) =>
      doc.agentId === agentId && (!coord || (doc.coord.x === coord.x && doc.coord.y === coord.y)),
  });
...
export function useDnaThread(appId: string) {
  ...
  const mutations = useQuery(
    z.query.dnaMutations.where("appId", appId).orderBy("createdOn", "desc").related("visualProof"),
  );
...
function coordToQuery<T extends Record<string, unknown>>(
  ...
  switch (coord.z) {
    ...
    case 1: // .sushi — project metadata, DNA
      return baseQuery.where("appId", coord.x).where("storeId", coord.y).where("category", "dna");
    ...
    case 3: // agents — xp, levels, memory pointers
      return baseQuery.where("appId", coord.x).where("storeId", coord.y).where("category", "agent");
    default:
...
const z = useZero();
const entities = useQuery(coordToQuery(currentCoord, z.query.entities));
```
...
const rootSub = z.query.apps.related("store").related("dnaMutations", (q) => q.limit(50)); // capped
```
