---
title: "Comprehensive spatial patent specification"
description: "Full patent specification for the cross-store polymorphic application system with spatial navigation, universal components, and domain-based context switching."
---
...
A polymorphic application system that generates independent applications across multiple platforms (web PWA, Chrome extension, desktop via Tauri, mobile via Capacitor) from a single codebase through:
...
**Live System:** Production deployment at https://chrry.ai with 12+ apps x 5 platforms = 60+ unique installable applications from one repository.
...
Result: 12 apps x 5 platforms = 60+ unique builds from one codebase.
...
# Tauri Desktop Builds (12 variants x 3 OS)
...
... x 12 apps = 36 desktop builds

# React Native Mobile (12 variants x 2 platforms)
MODE=atlas npm run build:ios → Atlas.ipa
MODE=atlas npm run build:android → Atlas.apk
... x 12 apps = 24 mobile builds
...
│  ├─ apps/                                                    │
│  │  ├─ chrry/        (PWA - 12 apps)                        │
│  │  ├─ extension/    (Chrome - 12 apps x 1 = 12)            │
│  │  ├─ browser/      (Tauri - 12 apps x 3 OS = 36)          │
│  │  └─ mobile/       (Capacitor - 12 apps x 2 = 24)         │
│  │                                                           │
│  └─ packages/                                                │
...
│  └────────────────────────────────────────────────────────┘ │
│                                                              │
│  RESULT: 6 platforms x 12 apps = 72 builds                  │
...
| -------------------------- | ------------- | ------------------ | --------------- | ------------------------------ |
| **Single codebase**        | No (one app)  | Yes (multi-service) | No (one app)    | Yes                             |
| **Cross-platform**         | No (web only) | No (containers)    | No (web only)   | Yes (web, native, ext, desktop) |
| **Spatial navigation**     | No            | No                 | No              | Yes (X/Y/Z axes)                |
| **App inheritance**        | No            | No                 | No              | Yes (extend parent apps)        |
| **Store hierarchy**        | No            | No                 | No              | Yes (nested stores)             |
| **Cross-store install**    | No            | No                 | No              | Yes (many-to-many)              |
| **Universal components**   | No            | No                 | No              | Yes (151+ components)           |
| **Dynamic builds**         | No            | No                 | No              | Yes (MODE env var)              |
| **Domain-based context**   | No            | No                 | No              | Yes (same app, diff behavior)   |
| **84+ apps from one repo** | No            | No                 | No              | Yes                             |
...
3. **Polymorphic builds**: 12 apps x 7 platforms = 84 unique applications
...
The **Sato Spatial Dojo** introduces a new approach to application architecture:
...
**The result:** A unified system where stores (domains), apps (contexts), and code (depth) form a 3D spatial navigation system enabling many variations from a shared codebase.
